...
function soldUser($connection, $user_id)
{
    ...
    SELECT SUM(
        CASE 
            WHEN c.type = 'revenu' THEN t.montant 
...
function detailsUser($connection, $user_id)
{
...
function totalIncomesByCategory($category, $connection, $user_id, $year, $month)
{
    $query = "
        SELECT SUM(t.montant) AS total_income
        FROM transactions t
...
function totalExpensesByCategory($category, $connection, $user_id, $year, $month)
{
    $query = "
        SELECT SUM(t.montant) AS total_expense
        FROM transactions t
