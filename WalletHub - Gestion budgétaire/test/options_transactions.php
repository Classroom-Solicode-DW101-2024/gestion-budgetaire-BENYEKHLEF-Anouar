...
<head>
    ...
    <title>Transactions - Gestion Budget</title>
    <link rel="stylesheet" href="./test/options_transactions.css?v=<?php echo time(); ?>">
...
<body>
    <div class="app-container">
        <aside class="sidebar">
            ...
            <nav class="nav-items">
                <a href="dashboard.php" class="nav-item" title="Tableau de bord"><i class="fa-solid fa-home"></i></a>
                <a href="transactions.php" class="nav-item active" title="Transactions"><i
                        class="fa-solid fa-exchange-alt"></i></a>
                <a href="categories.php" class="nav-item" title="Catégories"><i class="fa-solid fa-tags"></i></a>
        ...
        <div class="main-content">
            ...
            <?php if (isset($errors['general'])): ?>
                <p class="error"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['general']) ?>
                </p>
            <?php endif; ?>

            <button id="open-modal" class="btn primary"><i class="fa-solid fa-plus-circle"></i>&nbsp; Ajouter une transaction</button>
                    ...
                    <form action="transactions.php" method="post">
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-icon">
                                ...
                                <select name="type" id="type">
                                    <option value="" selected disabled <?= empty($_POST['type']) ? 'selected' : '' ?>>Choisir un type
                                    </option>
                                    <option value="revenu"
                                        <?= (isset($_POST['type']) && $_POST['type'] === 'revenu') ? 'selected' : '' ?>>
                                        Revenu</option>
                                    <option value="depense"
                                        <?= (isset($_POST['type']) && $_POST['type'] === 'depense') ? 'selected' : '' ?>>
                                        Dépense</option>
                                </select>
                            </div>
                            <?php if (isset($errors['type'])): ?>
                                <span class="error"><i class="fa-solid fa-circle-exclamation"></i>
                                    <?= htmlspecialchars($errors['type']) ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="input-group">
                            ...
                            <?php if (isset($errors['montant'])): ?>
                                <span class="error"><i class="fa-solid fa-circle-exclamation"></i>
                                    <?= htmlspecialchars($errors['montant']) ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="input-group">
                            ...
                            <?php if (isset($errors['category'])): ?>
                                <span class="error"><i class="fa-solid fa-circle-exclamation"></i>
                                    <?= htmlspecialchars($errors['category']) ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-icon">
                                ...
                                <textarea name="description"
                                    id="description"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-icon">
                                ...
                                <input type="date" name="date_transaction" id="date_transaction"
                                    value="<?= htmlspecialchars($_POST['date_transaction'] ?? '') ?>">
                            </div>
                            <?php if (isset($errors['date_transaction'])): ?>
                                <span class="error"><i class="fa-solid fa-circle-exclamation"></i>
                                    <?= htmlspecialchars($errors['date_transaction']) ?></span>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-icon">
                            ...
                            <select name="year" id="year">
                                <option value="" selected disabled>Toutes les années</option>
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-icon">
                            ...
                            <select name="month" id="month">
                                <option value="" selected disabled>Tous les mois</option>
                                <?php for ($m = 1; $m <= 12; $m++): ?>
                                    <option value="<?= $m ?>"
            ...
            <section class="transaction-history">
                ...
                <table border="0" cellpadding="5" cellspacing="0">
                    ...
                    <?php if (empty($transactionsList)): ?>
                        <tr>
                            <td colspan="6" style="text-align: center; padding: 2rem;">Aucune transaction pour cette
                                période.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($transactionsList as $trans): ?>
                            <tr>
                                <td><?= htmlspecialchars($trans['date_transaction']) ?></td>
                                ...
                                <td><i class="fa-solid fa-tags"></i>&nbsp; <?= htmlspecialchars($trans['category_name']) ?></td>
                                <td class="amount-<?= $trans['type'] ?>">
                                    <?= htmlspecialchars(number_format($trans['montant'], 2, ',', ' ')) ?> DH
                                </td>
                                <td><?= htmlspecialchars($trans['description'] ?: 'Sans description') ?></td>
                                <td>
    ...
    <script>
        ...
        function populateCategories() {
            ...
            if (selectedType && categories[selectedType]) {
                const catList = categories[selectedType];
                catList.forEach(cat => {
                    const option = document.createElement('option');
                    option.value = cat;
                    option.textContent = cat;
                    <?php if (isset($_POST['category'])): ?>
                        if (cat === '<?= htmlspecialchars($_POST['category']) ?>') option.selected = true;
                    <?php endif; ?>
