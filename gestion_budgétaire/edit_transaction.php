...
<head>
    ...
    <title>Modifier Transaction - Gestion Budget</title>
...
<body>
    <div class="app-container">
        <aside class="sidebar">
            ...
            <nav class="nav-items">
                ...
                <a href="transactions.php" class="nav-item active" title="Transactions"><i class="fa-solid fa-exchange-alt"></i></a>
                ...
                <form action="edit_transaction.php?id=<?= $id ?>" method="post">
                    <div class="input-group">
                        ...
                        <?php if (isset($errors['type'])): ?>
                            <span class="error"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['type']) ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="input-group">
                        ...
                        <div class="input-icon">
                            ...
                            <input type="number" step="0.01" name="montant" id="montant" value="<?= htmlspecialchars($_POST['montant'] ?? $transaction['montant']) ?>">
                        </div>
                        <?php if (isset($errors['montant'])): ?>
                            <span class="error"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['montant']) ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="input-group">
                        ...
                        <?php if (isset($errors['category'])): ?>
                            <span class="error"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['category']) ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="input-group">
                        ...
                        <div class="input-icon">
                            ...
                            <textarea name="description" id="description"><?= htmlspecialchars($_POST['description'] ?? $transaction['description']) ?></textarea>
                        </div>
                    </div>
                    <div class="input-group">
                        ...
                        <div class="input-icon">
                            ...
                            <input type="date" name="date_transaction" id="date_transaction" value="<?= htmlspecialchars($_POST['date_transaction'] ?? $transaction['date_transaction']) ?>">
                        </div>
                        <?php if (isset($errors['date_transaction'])): ?>
                            <span class="error"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['date_transaction']) ?></span>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        const existingType = "<?= htmlspecialchars($transaction['type'], ENT_QUOTES) ?>";
        const existingCategory = "<?= htmlspecialchars($transaction['category_name'], ENT_QUOTES) ?>";
        ...
        function populateCategories() {
            ...
            if (categories[typeSelect.value]) {
                categories[typeSelect.value].forEach(cat => {
                    ...
                    opt.value = cat;
                    opt.textContent = cat;
                    if (cat === existingCategory || (cat === "<?= htmlspecialchars($_POST['category'] ?? '') ?>")) {
                        opt.selected = true;
                    }
